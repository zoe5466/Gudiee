...
export default function EditPostPage() {
    ...
    const fetchPost = async () => {
      try {
        ...
        setFormData({
          ...
          status: post.status,
          embeddedServices: post.embeddedServices?.map((embed: any) => ({
            serviceId: embed.service.id,
            embedType: embed.embedType || 'card',
            customText: embed.customText || '',
          })) || [],
        })
      } catch (err) {
    ...
    return (
      <div className="flex items-center justify-center min-h-screen bg-[#cfdbe9]">
        <div className="text-center">
          <p className="text-gray-700 mb-4">{error || '無法載入貼文'}</p>
          <Link href="/my-content" className="text-[#002C56] hover:underline">
            返回
          </Link>
        </div>
      </div>
  ...
  const handleAddService = (serviceId: string) => {
    setFormData((prev) => {
      if (!prev || prev.embeddedServices.find((s) => s.serviceId === serviceId)) {
        return prev
      }
      return {
        ...prev,
